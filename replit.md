...
## Recent Changes
- **2025-11-19**: Version 3.0.6 released - NUCLEAR FIX: Implemented OverlayServiceGuard that completely disables OverlayService at OS level (PackageManager.setComponentEnabledSetting) until AI model is imported. Service literally cannot be started by ANY means (Quick Settings, notifications, system revival) while disabled. ModelProvisionOrchestrator disables service on startup, ImportActivity disables before file picker, both re-enable after successful import. This is the FINAL solution - Android physically cannot start the service during import. Ready for production build.
