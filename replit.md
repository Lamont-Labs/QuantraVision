...
## Recent Changes
- **2025-11-19**: Version 3.0.5 released - CRITICAL FIX: Added model state guards to ALL service start locations (ScanViewModel, AppScaffold) to prevent service from ever starting without imported model. ImportActivity now explicitly calls stopService() before launching file picker to kill any zombie service instances. Combined with v3.0.4's URI permission persistence. This is the nuclear option - multiple defense layers. Ready for production build.
