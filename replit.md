...
**QuantraBot AI Assistant**: Interactive AI trading assistant providing pattern validation, conversational explanations, and trading Q&A. Built on Gemma 2B with expert pattern knowledge base system (10+ patterns with validation rules, invalidation signals, common mistakes, and trade setups stored as JSON). Features QuantraBotEngine for coordinating chat/validation/explanations, PatternPromptBuilder for expert-level prompt engineering (teaches LLM via knowledge instead of expensive fine-tuning), PatternKnowledgeLoader for efficient knowledge caching, and Material 3 chat UI (QuantraBotScreen/ViewModel). Accessible via dedicated "Bot" tab in bottom navigation. Supports AI mode (when Gemma model available) and fallback mode (template-based responses). Uses applicationContext to prevent memory leaks. All processing 100% offline, privacy-preserving.
