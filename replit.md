...
## Recent Changes
- **2025-11-19**: Fixed MediaPipe API compatibility - corrected GemmaEngine to use proper MediaPipe 0.10.27 API: removed incorrect `BaseOptions` usage, removed non-existent `setResultListener()` call, implemented correct nested class `LlmInference.LlmInferenceOptions.builder()` with direct `setModelPath()` API
