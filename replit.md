...
## Recent Changes
- **2025-11-19**: Version 3.0.8 released - FINAL FIX: Fixed file access permissions and import flow. Added MANAGE_EXTERNAL_STORAGE permission for Android 11+ to access Download folder, implemented proper permission request flow, added direct file copy using FileInputStream/FileOutputStream (bypasses broken WorkManager approach), shows progress toasts, validates file exists and is readable, calls ModelManager.onModelDownloaded() to update state. User types file path → grants "All files access" permission → file imports successfully. No more crashes, no more immediate finish bug.
