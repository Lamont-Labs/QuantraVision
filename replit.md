...
**Background Design System**: All screens utilize a unified StaticBrandBackground composable with a simple dark blue-teal gradient (#010409 → #0D1825 → #010409) that perfectly matches the branded logo image background. This creates a clean, seamless visual experience across the entire app with no animations or visual noise. The static gradient ensures the branded hero logo image on the HomeScreen blends perfectly with the UI background, eliminating any visible borders or transitions.
