...
## Recent Changes
- **2025-11-19**: Version 3.0.9 released - Fixed crash when asking questions after model import. Root cause: QuantraBot was creating new AI engine instances instead of using the singleton, so after import the UI kept using the old uninitialized instance. Fixed by making QuantraBotEngine use GemmaEngine.getInstance() singleton, added explicit re-initialization of shared instance in refreshModelState(), added better null safety checks and error messages throughout the AI pipeline. Also fixed import path bugs (corrected method name from onModelDownloaded to onModelImported).
- **2025-11-19**: Version 3.0.8 released - Fixed file access permissions and import flow. Added MANAGE_EXTERNAL_STORAGE permission for Android 11+ to access Download folder, implemented proper permission request flow, added direct file copy using FileInputStream/FileOutputStream (bypasses broken WorkManager approach), shows progress toasts, validates file exists and is readable, calls ModelManager.onModelImported() to update state. User types file path → grants "All files access" permission → file imports successfully.
- **2025-11-19**: Version 3.0.1 released - Fixed NeonButton reference (component doesn't exist, changed to MetallicButton). Fixed exhaustive when expression (added missing ModelState branches). Fixed NeonText textAlign parameter (moved to style). Fixed critical accessibility bug in ModelImportScreen where "Import Model" button was invisible/inaccessible (refactored to use Scaffold with fixed bottomBar and window insets padding).
