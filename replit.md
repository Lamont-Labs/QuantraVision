...
## Recent Changes
- **2025-11-19**: Version 3.0.2 released - CRITICAL FIX: Added model state check in OverlayService.onCreate() and onStartCommand() to prevent crash when Android auto-restarts the service (START_STICKY). Service now gracefully stops itself if model isn't imported, preventing "app has a bug" crash on startup. Ready for production build.
