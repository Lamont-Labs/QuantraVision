...
## Recent Changes
- **2025-11-19**: Version 3.0.3 released - CRITICAL FIX: Fixed Android foreground service lifecycle violation. Now always calls startForeground() before any stopSelf() to comply with Android's foreground service contract. Added stopForeground(true) before all stopSelf() calls. This completely eliminates the "app has a bug" crash when service can't start due to missing model. Ready for production build.
