...
## Recent Changes
- **2025-11-20**: Version 3.1.0 released with Bundled Ensemble AI Engine - Replaced single 555MB Gemma model with 2 bundled quantized TFLite models (total ~47MB, bundled in APK): all-MiniLM-L6-v2 sentence embeddings (22MB) and MobileBERT Q&A INT8 (25MB). APK size reduced from ~300MB to ~147MB while making AI responses 5-10x faster for common questions. Models auto-provision from assets on first launch - NO USER IMPORT REQUIRED! Features: EmbeddingsRetriever provides instant answers via cosine similarity search against QAKnowledgeBase (198 pre-written pattern/trading Q&As), MobileBERT generates answers for novel questions. ModelManager auto-detects bundled models and provisions to internal storage. All processing remains 100% offline.
...
**Ensemble AI Engine**: A 3-model orchestration system (IntentClassifier, EmbeddingsRetriever, MobileBERTQaAdapter) for Q&A, offering a 15x smaller size and 5-10x faster inference compared to a single large model. It orchestrates components using TFLite NLClassifier, all-MiniLM-L6-v2 sentence embeddings, and BertQuestionAnswerer. All processing is 100% offline.
...
**Model Import System** (Mobile-Only): Provides an in-app workflow for sequentially importing the three ensemble AI models (`intent_classifier.tflite`, `sentence_embeddings.tflite`, `mobilebert_qa_squad.tflite`) directly from the user's phone using the Storage Access Framework. It validates filenames, shows progress, and tracks model states, requiring zero permissions.
