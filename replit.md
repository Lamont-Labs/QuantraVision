...
**Background Design System**: All screens utilize a unified QuantumGridBackground composable with a dark blue-teal gradient (#010409 → #0D1825 → #010409) that matches the branded logo image background. This creates a seamless visual experience across the entire app with animated grid lines, candlestick parallax depth effects, particle starfield overlays, and radial glow accents. The HomeScreen hero logo integrates a radial blend layer to eliminate visible borders and ensure the branded logo appears to float naturally within the UI.
